...
const ImpactPage = () => {
  ...
  // Memoize hero images to prevent re-creation on every render
  ...
  // Memoize all data to prevent re-creation
  ...
  const communityTransformations = useMemo(
    () => [
      {
  ...
  }, [heroImages.length, successStories.length])

  // Optimized AnimatedSection with better performance
  ...
  const CounterAnimation = React.memo(({ value, duration = 2 }: { value: string; duration?: number }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Hero Section with Smooth Changing Backgrounds */}
            ...
            {impactStats.map((stat, index) => (
              <AnimatedSection key={index} delay={index * 0.1}>
                <motion.div
                  ...
                  className={`${stat.bgColor} rounded-2xl p-8 text-center border border-gray-100 shadow-lg`}
                >
                  <div
      ...
      {/* Testimonials from Individuals and Organizations */}
            ...
            {futureGoals.map((goal, index) => (
              <AnimatedSection key={index} delay={index * 0.2}>
                <motion.div
                  ...
                  className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 text-center h-full"
                >
                  <div
                    className={`w-20 h-20 bg-gradient-to-r ${goal.color} rounded-full flex items-center justify-center mx-auto mb-6`}
                  >
                    {React.createElement(goal.icon, { className: "w-10 h-10 text-white" })}
                  </div>
